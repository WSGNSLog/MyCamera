...
- (void)hide:(BOOL)animated {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(shoeHideDelayed:) object:[NSNumber numberWithBool:animated]];//可以取消成功。
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(showHideDelayed:) object:[NSNumber numberWithBool:animated]];//可以取消成功。
}

- (void)hideH:(BOOL)animated {
    useAnimation = animated;
    // If the minShow time is set, calculate how long the hud was shown,
    // and pospone the hiding operation if necessary
    if (self.minShowTime > 0.0 && showStarted) {
...
- (void)hide:(BOOL)animated afterDelay:(NSTimeInterval)delay errorMessage:(NSString *)errorMessage
{
    [self performSelector:@selector(hideDelayed:) withObject:[NSNumber numberWithBool:animated] afterDelay:delay];
    if (delay==30) {
//        [self performSelector:@selector(shoeHideDelayed: errorMessage:) withObject:[NSNumber numberWithBool:animated] afterDelay:delay];
        self.errorMessage = [NSString stringWithString:errorMessage];
        [self performSelector:@selector(showHideDelayed:) withObject:[NSNumber numberWithBool:animated] afterDelay:delay];
    }
}
...
- (void)hide:(BOOL)animated afterDelay:(NSTimeInterval)delay {
	[self performSelector:@selector(hideDelayed:) withObject:[NSNumber numberWithBool:animated] afterDelay:delay];
    if (delay==30) {
        [self performSelector:@selector(shoeHideDelayed:) withObject:[NSNumber numberWithBool:animated] afterDelay:delay];
    }
}

- (void)hideDelayed:(NSNumber *)animated {
	[self hideH:[animated boolValue]];
}
-(void)shoeHideDelayed:(NSNumber *)animated{
    [MBProgressHUD showError:@"保存失败，请稍后重试"];
}
-(void)showHideDelayed:(NSNumber *)animated{
