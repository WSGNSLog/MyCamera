...
@interface ImageCutoutVC ()<UIScrollViewDelegate>
...
@property (weak, nonatomic) IBOutlet CutPathView *pHandleView;
...
-(void)initData{
    ...
    CGFloat space = 40;
    CGFloat WH = LL_ScreenWidth - space*2;
...
- (IBAction)shapeHandleBtnClicked:(UIButton *)sender {
    
    self.handleContentView.hidden = NO;
    self.pathHandleContentView.hidden = YES;
}
- (IBAction)pathHandleBtnClicked:(UIButton *)sender {
...
-(void)pathHandleContentViewPanGestureHanele:(UIPanGestureRecognizer*)panGesture{
    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            self.panLocationPoint = locationPoint;
            ///
            UIColor * color = [UIColor colorWithHexString:@"0xE4564A"];
